const DocumentService = (function () {
   ...
   */
  const convertToMarkdown = (spreadsheetId, options = {}) => {
    const safeId = getSafeWorkbookId(spreadsheetId);
    const spreadsheet = SpreadsheetApp.openById(safeId);
    ...
    let markdownContent = `# ${spreadsheet.getName()}\n\n`;
   ...
   * @returns {Object}
   */
  const convertToJSON = (spreadsheetId, options = {}) => {
    const safeId = getSafeWorkbookId(spreadsheetId);
    const spreadsheet = SpreadsheetApp.openById(safeId);
    let sheets = spreadsheet.getSheets();
   ...
   * @returns
   */
  const getSpreadsheetInfo = (id) => {
    const spreadsheetId = getSafeWorkbookId(id);
   ...
   *
   * @param {string} id
   * @returns {string} - The ID of the Google Spreadsheet (Google Sheets)
   */
  function getSafeWorkbookId(id) {
    const parentTempFolder = DriveApp.getFolderById(Config.WORKSPACES_FOLDER_ID);
